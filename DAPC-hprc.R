...
#names(data_t2t)[2] <- "population"
pop <- data_t2t$Superpopulation
data_t2t <- data_t2t[ -c(1,2) ]
dapc.data_t2t <-  dapc(data_t2t, grp=pop, var.contrib = TRUE, scale = FALSE, n.pca = 11, n.da = 100)

## Optimal PCs from a-score 
temp <- optim.a.score(dapc.data_t2t)
...
## Barplot
temp <- summary(dapc(data_t2t, grp=pop, var.contrib = TRUE, scale = FALSE, n.pca = 6, n.da = 100))$assign.per.pop*100
